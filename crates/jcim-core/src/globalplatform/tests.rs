...
use super::{
    ...
    initialize_update, parse_get_status, parse_initialize_update, select_issuer_security_domain,
    set_application_status, set_card_status, set_security_domain_status,
};
...
fn parses_get_status_response_with_tlvs() {
    ...
    let parsed = parse_get_status(RegistryKind::Applications, &response).expect("parse get status");
...
fn parses_more_data_status() {
    ...
    let parsed = parse_get_status(RegistryKind::IssuerSecurityDomain, &response).expect("parse");
    assert!(parsed.more_data_available);
}
...
fn parses_initialize_update_responses_for_scp02_and_scp03() {
    let scp02 = ResponseApdu::success(vec![
        0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x11, 0x02, 0xAA, 0x55, 0x10,
        0x11, 0x12, 0x13, 0x14, 0x15, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27,
    ]);
    ...
    let scp03 = ResponseApdu::success(vec![
        0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x12, 0x03, 0x70, 0x30, 0x31,
        0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47,
    ]);
